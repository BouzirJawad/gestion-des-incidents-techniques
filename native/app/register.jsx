...
const RegisterSchema = Yup.object().shape({
  ...
  phone: Yup.string().required('Numéro requis'),
...
export default function RegisterScreen() {
  ...
  const handleRegister = async (values) => {
    try {
      ...
      await axios.post('http://YOUR-IP:7460/api/auth/register', data);
  ...
  return (
    <Formik
      initialValues={{ username: '', email: '', phone: '', isAdmin: false, password: '', confirmPassword: '' }}
      validationSchema={RegisterSchema}
      onSubmit={handleRegister}
    >
      {({ handleChange, handleBlur, handleSubmit, values, setFieldValue, errors, touched }) => (
        <View className="flex-1 justify-center px-6 bg-[#F8F6F3]">
          <Text className="text-3xl font-bold text-center mb-6 text-[#3D5681]">Inscription</Text>
          ...
          >
            <Picker.Item label="Utilisateur" value={false} />
